...
@PacketHandler(support = {
        ControllerSecurityProto.InitAesKeyPacket.class
})
public class ControllerInitAesKeyPacketHandler implements BasePacketHandler {
    @Override
    public void handle(Channel channel, BaseProto.Packet packet, Message message) throws Exception {
        ControllerSecurityProto.InitAesKeyPacket initAesKeyPacket = (ControllerSecurityProto.InitAesKeyPacket) message;
        GlobalConfiguration.tempAesKey = new AesKey(
                AESUtils.getIv(initAesKeyPacket.getIv()),
                AESUtils.getKey(initAesKeyPacket.getKey())
        );

        NettyControllerInitAesKeyPacketCallback callback =
                NettyCallbackCollection.INSTANCE.getNettyCallback(ControllerInitAesKeyPacketHandler.class);
